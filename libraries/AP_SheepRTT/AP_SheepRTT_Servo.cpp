#include <AP_SheepRTT/AP_SheepRTT_Servo.h>
...
void AP_SheepRTT_Servo::init_sheepRTT()
{
    // move the servo to the release position
    release();
}

void AP_SheepRTT_Servo::grab()
{
    ...
    is_released = true;
#endif
    gcs().send_text(MAV_SEVERITY_INFO, "SheepRTT load grabbing");
    AP::logger().Write_Event(LogEvent::SHEEPRTT_START);
}

void AP_SheepRTT_Servo::release()
{
    // move the servo to the release position
    ...
    is_released = false;
#endif
    gcs().send_text(MAV_SEVERITY_INFO, "SheepRTT load releasing");
    AP::logger().Write_Event(LogEvent::SHEEPRTT_STOP);
}

bool AP_SheepRTT_Servo::has_state_pwm(const uint16_t pwm) const
{
...
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
    if (is_releasing) {
        gcs().send_text(MAV_SEVERITY_INFO, "SheepRTT load released");
    } else {
        gcs().send_text(MAV_SEVERITY_INFO, "SheepRTT load grabbed");
    }
#endif
    return true;
}
...
bool AP_SheepRTT_Servo::released() const
{
    return has_state_pwm(config.release_pwm);
}

bool AP_SheepRTT_Servo::grabbed() const
{
    return has_state_pwm(config.grab_pwm);
}

// type-specific periodic updates:
void AP_SheepRTT_Servo::update_sheepRTT() {
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
...
bool AP_SheepRTT_Servo::valid() const
{
    if (!AP_SheepRTT_Backend::valid()) {
        return false;
    }
